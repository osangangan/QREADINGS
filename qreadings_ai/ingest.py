...
headings that are explicitly present in the source.
"""
...
def ingest_markdown(path: Path, memory: Memory, root: Path) -> int:
    ...
    # Headings are explicit labels, so they are safe first-pass concepts.
...
def ingest_json(path: Path, memory: Memory, root: Path) -> int:
    ...
    for index, record in enumerate(records):
        ...
        # These are explicit corpus fields, not generated semantic guesses.
...
def ingest_repository(root: str | Path, db_path: str | Path = "qreadings_ai.db") -> dict[str, int]:
    root = Path(root).resolve()
    memory = Memory(db_path)
    stats = {"markdown_files": 0, "json_files": 0, "evidence_chunks": 0, "json_records": 0}
    try:
...
def main() -> None:
    ...
    stats = ingest_repository(args.root, args.db)
